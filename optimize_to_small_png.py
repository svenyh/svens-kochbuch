...
MAX_SIZE = (1000, 1000)  # kleiner = deutlich weniger Dateigröße
PNG_COLORS = 128         # weniger Farben = kleinere PNGs
...
def optimize_to_small_png(src_file: Path, src_root: Path, dst_root: Path) -> None:
    ...
    with Image.open(src_file) as img:
        ...
        if img.mode == "RGBA":
            img = img.convert("P", palette=Image.Palette.ADAPTIVE, colors=PNG_COLORS)
        else:
            img = img.convert("P", palette=Image.Palette.ADAPTIVE, colors=PNG_COLORS)
...
def main() -> None:
    if len(sys.argv) < 3:
        print("Verwendung:")
        print("python optimize_to_small_png.py <eingabeordner> <ausgabeordner>")
        print("Beispiel:")
        print("python optimize_to_small_png.py images images_png_small")
        sys.exit(1)
    ...
    print(f"Starte aggressive PNG-Optimierung für {len(files)} Datei(en)...")
    ...
    for file_path in files:
        try:
            optimize_to_small_png(file_path, src_root, dst_root)
