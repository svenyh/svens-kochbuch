...
def optimize_to_medium_png(src_file: Path, src_root: Path, dst_root: Path) -> None:
...
def main() -> None:
    if len(sys.argv) < 3:
        print("Verwendung:")
        print("python optimize_to_png_medium.py <eingabeordner> <ausgabeordner>")
        print("Beispiel:")
        print("python optimize_to_png_medium.py images images_png_medium")
        sys.exit(1)
    ...
    print(f"Starte mittlere PNG-Optimierung für {len(files)} Datei(en)...")
    ...
    for file_path in files:
        try:
            optimize_to_medium_png(file_path, src_root, dst_root)
